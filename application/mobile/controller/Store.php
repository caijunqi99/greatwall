...
class Store extends MobileMall {
    ...
    public function store_info() {
        ...
        // 页头背景图
        $store_info['mb_title_img'] = isset($store_online_info['mb_title_img']) ? UPLOAD_SITE_URL . '/' . ATTACH_STORE . '/' . $store_online_info['mb_title_img'] : '';
    ...
    public function store_goods() {
        ...
        $stc_id = isset($param['stc_id'])?(int) $param['stc_id']:'';
        ...
        $model_goods = Model('goods');
        $goods_fields = $this->getGoodsFields();
        $goods_list = $model_goods->getGoodsListByColorDistinct($condition, $goods_fields, $order, 10);
        $goods_list = $this->_goods_list_extend($goods_list);
        output_data(array('goods_list_count' => count($goods_list),'goods_list' => $goods_list,), mobile_page($model_goods->page_info));
    }
    ...
    public function store_new_goods() {
        ...
        $model_goods = Model('goods');
        $goods_fields = $this->getGoodsFields();
        $goods_list = $model_goods->getGoodsListByColorDistinct($where, $goods_fields, $order, $this->pagesize);
        if ($goods_list) {
            $goods_list = $this->_goods_list_extend($goods_list);
    ...
    public function store_intro() {
        ...
        $store_info['store_collect'] = (int) $store_online_info['store_collect'];
        //店铺所属分类
        $store_class = Model('storeclass')->getStoreClassInfo(array('sc_id' => $store_info['sc_id']));
