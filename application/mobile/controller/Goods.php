...
//        $_GET['is_book'] = 0;
        ...
        //优先从全文索引库里查找
        list($goods_list, $indexer_count) = $model_search->indexerSearch($indexer_searcharr, $this->pagesize);
        if (!is_null($goods_list)) {
            ...
            $goods_list = $model_goods->getGoodsListByColorDistinct($condition, $fieldstr, $order, $this->pagesize);
        }
        //处理商品列表(团购、限时折扣、商品图片)
        $goods_list = $this->_goods_list_extend($goods_list);

        output_data(array('goods_list' => $goods_list), mobile_page(is_object($model_goods->page_info)?$model_goods->page_info:''));
    }
    ...
    private function _goods_list_extend1($goods_list) {
        ...
        $sole_array = Model('psole')->getSoleGoodsList(array('goods_id' => array('in', $goodsid_array)));
    ...
    public function goods_evaluate() {
        ...
        $goods_eval_list = \think\Loader::model('memberevaluate','logic')->evaluateListDity($goods_eval_list);
        output_data(array('goods_eval_list' => $goods_eval_list), mobile_page( is_object($model_evaluate_goods->page_info)?$model_evaluate_goods->page_info:0));
    }
