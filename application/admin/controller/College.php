 ...
 * 控制器
 */
class College extends AdminControl {
            ...
            if ($article_id) {
                ...
                if (!empty($_FILES['_pic']['name'])) {
                    ...
                    if ($result) {
                        ...
                        // 上传失败获取错误信息
                        $this->error($file->getError(),url('College/edit',['article_id'=>$article_id]));
                    }
                }
                $this->success(lang('ds_common_save_succ'), 'College/college');
            } else {
        ...
        if (!request()->isPost()) {
            ...
            if (!empty($_FILES['_pic']['name'])) {
                ...
                if ($result) {
                    ...
                    // 上传失败获取错误信息
                    $this->error($file->getError(), url('College/edit', ['article_id' => $result]));
                }
            }
            //验证数据  END
    ...
    protected function getAdminItemList() {
        ...
        if (request()->action() == 'add' || request()->action() == 'college') {
            $menu_array[] = array(
                'name' => 'add',
                'text' => lang('ds_new'),
                'url' => url('College/add')
