...
class Waybill extends Model
{
     ...
     * @return array
     */
    public function getWaybillList($condition, $pagesize='', $order='waybill_usable desc', $field='*') {
        $waybill =  db('waybill')->field($field)->where($condition)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
     ...
     * @return type
     */
    public function getWaybillAdminList($pagesize = '') {
        $condition = array();
        $condition['store_id'] = 0;
        return $this->getWaybillList($condition, $pagesize);
    }
    ...
    public function getPrintInfoByOrderInfo($order_info) {
        ...
        $print_info['seller_name'] = !empty($daddress_info)?$daddress_info['seller_name']:'';
        $print_info['seller_area'] = !empty($daddress_info)?$daddress_info['area_info']:'';
        $print_info['seller_address'] = !empty($daddress_info)?$daddress_info['daddress_detail']:'';
        $print_info['seller_phone'] = !empty($daddress_info)?$daddress_info['daddress_telphone']:'';
        $print_info['seller_company'] = !empty($daddress_info)?$daddress_info['daddress_company']:'';
